                ...
                <el-collapse-item title="老带新信息" name="10">
                    <el-col :span="8">
                        <el-form-item label="推荐人：">
                            <el-input v-model="command.customerId" placeholder="请输入推荐人"></el-input>
                        </el-form-item>
                    </el-col>
                    <el-col :span="8">
    ...
    export default {
        ...
        async mounted() {
            ...
            if (this.contractId != undefined) {
                ...
                if (this.contractTemporary) {
                    ...
                    // 加载楼栋房型等信息
                    this.changeApartment(command.apartmentId, true);
                    if (command.pricePackageId) {
                        this.changePricePackage(command.pricePackageId);
                    ...
                    const command = (await this.$axios.get("contract/" + this.contractId)).data;
                    command.bArea = this.stringToArray(command.bArea);
                    command.cArea = this.stringToArray(command.cArea);
                    command.cLiveArea = this.stringToArray(command.cLiveArea);
                    command.cLegalPersonArea = this.stringToArray(command.cLegalPersonArea);
                    command.dArea = this.stringToArray(command.dArea);
                    command.dLiveArea = this.stringToArray(command.dLiveArea);
                    command.dLegalPersonArea = this.stringToArray(command.dLegalPersonArea);
                    this.command = command;
        ...
        methods: {
            ...
            changeApartment(apartmentId, notReset) {
                ...
                console.log(this.selectOption.pricePackages);
